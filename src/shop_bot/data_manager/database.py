        ...
        with sqlite3.connect(DB_FILE) as conn:
            ...
            cursor.execute('''
                CREATE TABLE IF NOT EXISTS xui_hosts(
                    ...
                    remna_cookie TEXT,
                    remna_cf_client_id TEXT,
                    remna_cf_client_secret TEXT,
                    host_order INTEGER,
...
def run_migration():
    ...
    try:
        ...
        if table_exists:
            ...
            if 'remna_cookie' not in xh_columns:
                cursor.execute("ALTER TABLE xui_hosts ADD COLUMN remna_cookie TEXT")
                logging.info(" -> Столбец 'remna_cookie' успешно добавлен в 'xui_hosts'.")
            if 'remna_cf_client_id' not in xh_columns:
                cursor.execute("ALTER TABLE xui_hosts ADD COLUMN remna_cf_client_id TEXT")
                logging.info(" -> Столбец 'remna_cf_client_id' успешно добавлен в 'xui_hosts'.")
            if 'remna_cf_client_secret' not in xh_columns:
                cursor.execute("ALTER TABLE xui_hosts ADD COLUMN remna_cf_client_secret TEXT")
                logging.info(" -> Столбец 'remna_cf_client_secret' успешно добавлен в 'xui_hosts'.")
...
def create_host(
    ...
    remna_cookie: str | None = None,
    remna_cf_client_id: str | None = None,
    remna_cf_client_secret: str | None = None,
):
    try:
        ...
        remna_cookie = (remna_cookie or "").strip() or None
        remna_cf_client_id = (remna_cf_client_id or "").strip() or None
        remna_cf_client_secret = (remna_cf_client_secret or "").strip() or None
        try:
            ...
            try:
                cursor.execute(
                    ...
                    INSERT INTO xui_hosts (
                        ...
                        remna_api_token, remna_caddy_api_key, remna_cookie,
                        remna_cf_client_id, remna_cf_client_secret,
...
def update_host_remna_cookie(host_name: str, remna_cookie: str | None) -> bool:
    try:
        host_name = normalize_host_name(host_name)
        value = (remna_cookie or "").strip() or None
        with sqlite3.connect(DB_FILE) as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT 1 FROM xui_hosts WHERE TRIM(host_name) = TRIM(?)", (host_name,))
            if cursor.fetchone() is None:
                logging.warning(f"update_host_remna_cookie: хост '{host_name}' не найден")
                return False
            cursor.execute(
                "UPDATE xui_hosts SET remna_cookie = ? WHERE TRIM(host_name) = TRIM(?)",
                (value, host_name)
            )
            conn.commit()
            return True
    except sqlite3.Error as e:
        logging.error(f"Не удалось обновить remna_cookie для хоста '{host_name}': {e}")
        return False

def update_host_remna_cf_access(host_name: str, client_id: str | None, client_secret: str | None) -> bool:
    try:
        host_name = normalize_host_name(host_name)
        cid = (client_id or "").strip() or None
        secret = (client_secret or "").strip() or None
        with sqlite3.connect(DB_FILE) as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT 1 FROM xui_hosts WHERE TRIM(host_name) = TRIM(?)", (host_name,))
            if cursor.fetchone() is None:
                logging.warning(f"update_host_remna_cf_access: хост '{host_name}' не найден")
                return False
            cursor.execute(
                """
                UPDATE xui_hosts
                SET remna_cf_client_id = ?, remna_cf_client_secret = ?
                WHERE TRIM(host_name) = TRIM(?)
                """,
                (cid, secret, host_name)
            )
            conn.commit()
            return True
    except sqlite3.Error as e:
        logging.error(f"Не удалось обновить Cloudflare Access для хоста '{host_name}': {e}")
        return False
