        ...
        if is_expired_host:
            try:
                ...
                for user in all_users:
                    ...
                    if not user_id:
                        continue
                    user_keys = database.get_user_keys(int(user_id)) or []
                    if not user_keys:
                        continue
                    ...
                    result = await xui_api.create_or_update_key_on_host(
                        host_name=host_name,
                        email=email,
                        expiry_timestamp_ms=max_expiry_ms,
                        preferred_uuid=database.get_or_create_user_subscription_uuid(int(user_id))
                    )
                    if result:
                        await xui_api.set_client_enabled_on_host(
                            host_name,
                            email,
        ...
        try:
            ...
            for db_key in keys_in_db:
                ...
                if server_client:
                    ...
                    # Keep keys in DB, but disable only expired clients on panel.
                    if is_expired:
                        try:
                            await xui_api.set_client_enabled_on_host(host_name, key_email, enabled=False)
                        except Exception as e:
                            logger.warning(f"Scheduler: Не удалось отключить истекший ключ '{key_email}' на '{host_name}': {e}")
                    else:
                        try:
                    ...
                    # Enforce monthly traffic reset on hosts with traffic cap.
                    try:
                        traffic_limit = float(host.get("client_monthly_traffic_gb") or 0)
                    except Exception:
                        traffic_limit = 0.0
                    if traffic_limit > 0:
                        try:
                            await xui_api.set_client_monthly_reset_on_host(host_name, key_email, reset_days=30)
                        except Exception as e:
                            logger.warning(f"Scheduler: Не удалось установить monthly reset для '{key_email}' на '{host_name}': {e}")
                else:
                    logger.warning(
                        f"Scheduler: Ключ '{key_email}' для хоста '{host_name}' не найден на сервере. "
                        "Запись оставлена в БД, удаление отключено."
                    )
