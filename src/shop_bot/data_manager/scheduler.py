        ...
        if is_expired_host:
            try:
                ...
                for user in all_users:
                    ...
                    if result:
                        try:
                            await xui_api.set_client_enabled_on_host(host_name, email, enabled=True)
                        except Exception:
                            pass
            except Exception as e:
        ...
        try:
            ...
            for db_key in keys_in_db:
                ...
                if server_client:
                    ...
                    # On expiry, keep client untouched on panel:
                    # no delete, no disable. Renewal will update expiry for the same key.
                    if is_expired:
                        # Expired key is handled; skip further per-key processing.
                        continue
                    else:
                        try:
