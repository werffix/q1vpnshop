            ...
            if traffic_bytes is not None:
                for attr in ("total_gb", "totalGB"):
                    try:
                        ...
                        pass
                try:
                    inbound_to_modify.settings.clients[client_index].reset = 1
                except Exception:
                    pass
...
async def get_key_usage_stats_from_host(key_data: dict) -> dict:
    ...
    if total_bytes <= 0:
        total_bytes = _as_int(_obj_get(match, "totalGB"))
    if total_bytes <= 0:
        total_bytes = _as_int(_obj_get(match, "total_gb"))
