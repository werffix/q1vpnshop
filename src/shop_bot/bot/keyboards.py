...
def create_connect_landing_keyboard(connect_url: str) -> InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
    if connect_url:
        builder.button(text="🌐 Открыть личный кабинет", url=connect_url)
    builder.button(text="📱 Выбрать устройство", callback_data="show_connect_devices")
    builder.button(text=(get_setting("btn_back_to_menu") or "⬅️ Назад в меню"), callback_data="back_to_main_menu")
    builder.adjust(1)
    return builder.as_markup()
