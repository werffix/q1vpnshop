...
def create_platform_download_keyboard(
    ...
    # Activation button must always open fixed HTTPS url-wrapper with Happ deeplink.
    if subscription_url:
        try:
            ...
            if token:
                activate_url = (
                    "https://q1.servernux.com:8443/url"
                    f"?url=happ://add/https://q1.servernux.com:8443/sub/{token}"
                )
                builder.button(
