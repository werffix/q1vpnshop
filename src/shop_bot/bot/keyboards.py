...
def create_main_menu_keyboard(
    ...
    layout = [
        ...
        2,  # профиль + мои ключи
        2,  # подключиться/купить подписку + пополнить баланс
        1,  # рефералка
...
def create_profile_keyboard() -> InlineKeyboardMarkup:
    ...
    if kb:
        has_sub_button = any(
            (getattr(btn, "callback_data", None) == "profile_subscription_link")
        ...
        if has_sub_button and has_traffic_button:
            return kb
        builder = InlineKeyboardBuilder()
        for row in kb.inline_keyboard:
            builder.row(*row)
        if not has_sub_button:
            builder.button(text="🔗 Моя ссылка подписки", callback_data="profile_subscription_link")
        if not has_traffic_button:
            builder.button(text="📦 Докупить трафик", callback_data="buy_traffic_start")
        builder.adjust(1)
        return builder.as_markup()
...
def create_platform_download_keyboard(platform: str) -> InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
    p = (platform or "").lower()
    if p == "android":
        builder.button(
    ...
    builder.button(text="🔙 К устройствам", callback_data="show_connect_menu")
    builder.button(text=(get_setting("btn_back_to_menu") or "⬅️ Назад в меню"), callback_data="back_to_main_menu")
    builder.adjust(1)
    return builder.as_markup()
