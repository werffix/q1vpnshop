...
async def _build_happ_activation_url(subscription_url: str | None) -> str | None:
    ...
    headers = {
        ...
        "Referer": "https://www.happ.su/main/dev-docs/crypto-link",
    }
    try:
        async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=8)) as session:
            for endpoint in api_endpoints:
                try:
                    async with session.post(endpoint, json={"url": payload_url}, headers=headers) as resp:
                        body = await resp.text()
                        if resp.status >= 400:
                            logger.warning(
                                f"Happ crypto-link API HTTP {resp.status} on {endpoint}: {body[:180]}"
                            )
                            continue
                        if not body:
                            continue
                        # Response may be plain string or JSON.
                        text = body.strip()
                    ...
                    logger.warning(f"Happ crypto-link request failed on {endpoint}: {e}")
    except Exception as e:
