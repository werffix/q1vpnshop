...
def get_user_router() -> Router:
    ...
    async def topup_pay_platega(callback: types.CallbackQuery, state: FSMContext):
        ...
        result = await _platega_create_for_method_candidates(
            ...
            expected_markers=selected["expected"],
            success_url=success_url,
        ...
        if not result:
            await _safe_edit_or_send(
                callback.message,
                f"❌ Platega не подтвердила метод «{selected['method']}». "
                "Проверьте коды методов в настройках Platega и активность метода в мерчанте."
            )
            await state.clear()
            return
        ...
        await state.clear()
        if selected["expected"] and not any(m in (method_name_used or "") for m in selected["expected"]):
            logger.warning(
                "Platega topup: выбранный тип '%s' не подтверждён провайдером (method=%s, code=%s)",
    ...
    async def create_platega_payment_handler(callback: types.CallbackQuery, state: FSMContext):
        ...
        if selected["expected"] and not any(m in (method_name_used or "") for m in selected["expected"]):
            logger.warning(
                "Platega subscription: выбранный тип '%s' не подтверждён провайдером (method=%s, code=%s)",
                selected["method"], method_name_used, method_code_used
            )
...
async def _platega_create_for_method_candidates(
    ...
    expected = [m.upper() for m in expected_markers if m]
    last_ok: Optional[tuple[str, str, int, str]] = None
    matched = False
    ...
    for method_code in candidates:
        ...
        last_ok = (redirect_url, tx_id, method_code, method_name)
        if not expected:
            return last_ok
        if any(marker in (method_name or "") for marker in expected):
            matched = True
            return last_ok
