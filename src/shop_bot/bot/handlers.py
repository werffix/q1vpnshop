...
def _get_connect_page_url_for_user(user_id: int) -> str | None:
    sub_url = _get_unified_subscription_url_for_user(user_id)
    if not sub_url:
        return None
    try:
        parsed = urlparse(sub_url)
        token = parsed.path.rsplit("/", 1)[-1].strip().strip("<>")
        if not token:
            return None
        host = (parsed.hostname or "").strip()
        if not host:
            return None
        # Unified connect URL must always use 8443 as requested.
        netloc = f"{host}:8443"
        new_path = f"/connect/{token}"
        return urlunparse((parsed.scheme or "https", netloc, new_path, "", "", ""))
    except Exception:
        return None
...
def get_user_router() -> Router:
    ...
    async def show_connect_menu_handler(callback: types.CallbackQuery):
        await callback.answer()
        connect_url = _get_connect_page_url_for_user(callback.from_user.id) or "https://q1.servernux.com/connect/token"
        await _safe_edit_or_send(
            callback.message,
            "Откройте личный кабинет подключения:",
            reply_markup=keyboards.create_connect_landing_keyboard(connect_url)
        )

    @user_router.callback_query(F.data == "show_connect_devices")
    @registration_required
    async def show_connect_devices_handler(callback: types.CallbackQuery):
        await callback.answer()
        await _safe_edit_or_send(
            callback.message,
