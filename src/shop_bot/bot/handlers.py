...
def get_user_router() -> Router:
    ...
    async def process_trial_key_creation(message: types.Message):
        ...
        try:
            # Базовый local-part для email-ов ключей на разных хостах
            ...
            for host_name in host_names:
                host_email = _make_unique_email(base_local, host_name)
                result = await xui_api.create_or_update_key_on_host(
                    host_name=host_name,
                    email=host_email,
...
async def process_successful_payment(bot: Bot, metadata: dict):
    ...
    try:
        ...
        if action == "new":
            # Сформируем базовый local-part для генерации email на каждый хост
            user_data = get_user(user_id) or {}
            raw_username = (user_data.get('username') or f'user{user_id}').lower()
            username_slug = re.sub(r"[^a-z0-9._-]", "_", raw_username).strip("_")[:16] or f"user{user_id}"
            base_local = username_slug
        else:
        ...
        if action == "new":
            ...
            for current_host_name in host_names:
                host_email = _make_unique_email(base_local, current_host_name)
            ...
            if not successful_results:
                await processing_message.edit_text("❌ Не удалось создать подписку на серверах.")
                return
