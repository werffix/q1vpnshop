...
def get_user_router() -> Router:
    ...
    async def topup_pay_platega(callback: types.CallbackQuery, state: FSMContext):
        ...
        result = await _platega_create_transaction(
        ...
        if not result:
            await _safe_edit_or_send(callback.message, "❌ Не удалось создать ссылку Platega. Попробуйте другой способ оплаты.")
            await state.clear()
            return

        pay_url, transaction_id = result
        metadata = {
        ...
        await _safe_edit_or_send(
            callback.message,
            "Нажмите на кнопку ниже для оплаты. После оплаты нажмите 'Проверить оплату':",
            reply_markup=keyboards.create_payment_with_check_keyboard(pay_url, f"check_platega_{transaction_id}")
        )
    ...
    async def create_platega_payment_handler(callback: types.CallbackQuery, state: FSMContext):
        ...
        try:
            platega_method = int((get_setting(selected["setting"]) or str(selected["fallback"])).strip())
        except Exception:
            platega_method = int(selected["fallback"])
