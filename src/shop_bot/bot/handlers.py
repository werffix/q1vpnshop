...
async def _ensure_expired_hosts_clients(user_id: int, expiry_timestamp_ms: int, user_uuid: str):
    ...
    for host in expired_hosts:
        ...
        try:
            result = await xui_api.create_or_update_key_on_host(
                host_name=host_name,
                email=host_email,
                expiry_timestamp_ms=int(expiry_timestamp_ms),
                preferred_uuid=user_uuid
            )
            if result:
                try:
                    await xui_api.set_client_enabled_on_host(host_name, host_email, enabled=False)
                except Exception:
                    pass
        except Exception as e:
            logger.warning(
                f"Не удалось подготовить отключенный client для user={user_id} на expired-host '{host_name}': {e}"
            )
...
def get_user_router() -> Router:
    ...
    async def process_trial_key_creation(message: types.Message):
        ...
        try:
            ...
            if final_expiry_ms > 0:
                await _ensure_expired_hosts_clients(user_id, final_expiry_ms, user_uuid)
...
async def process_successful_payment(bot: Bot, metadata: dict):
    ...
    try:
        ...
        max_expiry_ms = max(
            [int(item[1].get("expiry_timestamp_ms") or 0) for item in successful_results] or [0]
        )
        if max_expiry_ms > 0:
            await _ensure_expired_hosts_clients(user_id, max_expiry_ms, user_uuid)
        if created_key_ids:
