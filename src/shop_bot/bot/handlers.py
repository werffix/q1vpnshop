...
def get_user_router() -> Router:
    ...
    @user_router.callback_query(F.data == "referral_share_story")
    @registration_required
    async def referral_share_story_handler(callback: types.CallbackQuery):
        await callback.answer()
        user_id = callback.from_user.id
        bot_username = (await callback.bot.get_me()).username
        referral_link = f"https://t.me/{bot_username}?start=ref_{user_id}"

        story_image_path = os.path.abspath(
            os.path.join(os.path.dirname(__file__), "..", "..", "..", "logo", "tgstories.jpg")
        )

        caption = (
            "📸 <b>Карточка для Stories готова</b>\n\n"
            f"Ваша реферальная ссылка:\n<code>{referral_link}</code>\n\n"
            "Опубликуйте это изображение в сторис и добавьте ссылку в описание."
        )

        try:
            if os.path.isfile(story_image_path):
                await callback.message.answer_photo(
                    photo=FSInputFile(story_image_path),
                    caption=caption,
                    reply_markup=keyboards.create_referral_keyboard(referral_link),
                )
            else:
                await callback.message.answer(
                    "⚠️ Файл для сторис не найден: logo/tgstories.jpg\n\n"
                    f"Ваша реферальная ссылка:\n<code>{referral_link}</code>",
                    reply_markup=keyboards.create_referral_keyboard(referral_link),
                    disable_web_page_preview=True,
                )
        except Exception as e:
            logger.error(f"Не удалось отправить материал для stories user={user_id}: {e}", exc_info=True)
            await callback.message.answer(
                "❌ Не удалось подготовить материал для stories. Попробуйте позже.",
                reply_markup=keyboards.create_referral_keyboard(referral_link),
            )
                ...
                try:
                    ...
                    if raw_expiry:
                        ...
                        if existing_expiry_dt > base_expiry_dt:
                            ...
                            host_email = _make_unique_email(f"u{user_id}.renew", current_host_name)
                            existing_key = None
                            local_rotated = True
                    else:
                        should_rotate_sub_id = True
