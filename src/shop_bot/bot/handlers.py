...
from shop_bot.data_manager.database import (
    ...
    get_plans_for_host, get_all_plans, get_plan_by_id, log_transaction, get_referral_count,
...
def _get_primary_host_with_plans() -> tuple[str | None, list]:
    hosts = get_all_hosts() or []
    if not hosts:
        return None, []

    global_plans = get_all_plans() or []
    if global_plans:
...
def get_user_router() -> Router:
    ...
    async def profile_handler_callback(callback: types.CallbackQuery):
        ...
        reward_type = (get_setting("referral_reward_type") or "").strip()
        if reward_type == "bonus_days_start":
            try:
                days_per_ref = int(get_setting("referral_on_start_referrer_days") or "0")
            except Exception:
                days_per_ref = 0
            referral_days = max(0, referral_count * days_per_ref)
        else:
            referral_days = 0
        ...
        final_text = (
            ...
            f"💰 <b>Заработано дней по рефералке:</b> {referral_days}"
        )
    ...
    async def referral_program_handler(callback: types.CallbackQuery):
        ...
        await callback.message.edit_text(
            text, reply_markup=keyboards.create_referral_keyboard(referral_link)
        )
    ...
    async def process_trial_key_creation(message: types.Message):
        ...
        if not host_names:
            await message.edit_text("❌ В данный момент нет доступных серверов для создания пробной подписки.")
            return
        ...
        try:
            ...
            if not results:
                await message.edit_text("❌ Не удалось создать пробную подписку. Проверьте настройки хостов.")
                return
            ...
            final_text = get_purchase_success_text(
                ...
                new_expiry_date,
                unified_url or results[0][1]['connection_string']
            )
            ...
            try:
                reply_kb = keyboards.create_key_info_keyboard(created_key_ids[0]) if created_key_ids else keyboards.create_back_to_menu_keyboard()
                await message.edit_text(text=final_text, reply_markup=reply_kb, disable_web_page_preview=True)
            except TelegramBadRequest:
    ...
    async def howto_android_handler(callback: types.CallbackQuery):
        await callback.answer()
        sub_url = _get_unified_subscription_url_for_user(callback.from_user.id) or "https://q1.servernux.com:8443/sub/token"
        try:
            await callback.message.edit_text(
                ...
                reply_markup=keyboards.create_platform_download_keyboard("android"),
                disable_web_page_preview=True
            )
        except TelegramBadRequest:
            pass
    ...
    async def howto_ios_handler(callback: types.CallbackQuery):
        ...
        try:
            await callback.message.edit_text(
                ...
                "3. Всё готово! Теперь вы можете выбрать локацию и подключиться!\n\n"
                f"🔗 <b>Subscription URL:</b>\n{html.code(sub_url)}",
                reply_markup=keyboards.create_platform_download_keyboard("ios"),
                disable_web_page_preview=True
            )
        except TelegramBadRequest:
            pass
    ...
    async def howto_windows_handler(callback: types.CallbackQuery):
        ...
        try:
            await callback.message.edit_text(
                ...
                "3. Всё готово! Теперь вы можете выбрать локацию и подключиться!\n\n"
                f"🔗 <b>Subscription URL:</b>\n{html.code(sub_url)}",
                reply_markup=keyboards.create_platform_download_keyboard("windows"),
                disable_web_page_preview=True
            )
        except TelegramBadRequest:
            pass
    ...
    async def howto_linux_handler(callback: types.CallbackQuery):
        ...
        try:
            await callback.message.edit_text(
                ...
                "3. Всё готово! Теперь вы можете выбрать локацию и подключиться!\n\n"
                f"🔗 <b>Subscription URL:</b>\n{html.code(sub_url)}",
                reply_markup=keyboards.create_platform_download_keyboard("linux"),
                disable_web_page_preview=True
            )
        except TelegramBadRequest:
            pass
    ...
    async def buy_new_key_handler(callback: types.CallbackQuery):
        ...
        hosts = get_all_hosts()
        if not hosts:
            await callback.message.edit_text("❌ В данный момент подписка недоступна.")
            return
    ...
    async def buy_traffic_start_handler(callback: types.CallbackQuery):
        ...
        if not host_name:
            ...
            return
        if not plans:
            await callback.message.edit_text("❌ Для продления пока не настроены тарифы.")
            return
        await callback.message.edit_text(
    ...
    async def buy_subscription_start_handler(callback: types.CallbackQuery):
        ...
        if not host_name:
            ...
            return
        if not plans:
            await callback.message.edit_text("❌ Для покупки пока не настроены тарифы.")
            return
        await callback.message.edit_text(
            CHOOSE_PLAN_MESSAGE or "Выберите тариф:",
            reply_markup=keyboards.create_plans_keyboard(plans, action="new", host_name=host_name)
    ...
    async def extend_key_handler(callback: types.CallbackQuery):
        ...
        if not host_name:
            await callback.message.edit_text("❌ Ошибка: Не удалось определить параметры подписки. Обратитесь в поддержку.")
            return
    ...
    async def plan_selection_handler(callback: types.CallbackQuery, state: FSMContext):
        ...
        if payload.startswith("buy:"):
            ...
            parts = payload.split(":")
            if len(parts) != 5:
                await callback.message.edit_text("❌ Ошибка выбора тарифа. Повторите попытку.")
                return
            ...
            try:
                ...
                await callback.message.edit_text("❌ Ошибка выбора тарифа. Повторите попытку.")
                return
            host = keyboards.find_host_by_callback_token(get_all_hosts() or [], host_token)
...
async def process_successful_payment(bot: Bot, metadata: dict):
    ...
    try:
        ...
        for current_host_name in host_names:
            host_email = _subscription_email_for_user_host(user_id, current_host_name)
        ...
        if action == "renewdays":
            days_added = int(months * 30)
            await bot.send_message(
                chat_id=user_id,
                text=f"✅ Вы успешно продлили свою подписку на {days_added} дней.",
                reply_markup=keyboards.create_back_to_menu_keyboard()
            )
        else:
            final_text = get_purchase_success_text(
            ...
            final_text += (
                ...
                "1. Нажмите «Скопировать подписку».\n"
                "2. Откройте клиент (Happ, v2rayNG, Shadowrocket, Streisand).\n"
                "3. Добавьте subscription URL и обновите список серверов."
            )

            await bot.send_message(
                chat_id=user_id,
                text=final_text,
                reply_markup=keyboards.create_subscription_result_keyboard(connection_string or "", key_id)
            )
