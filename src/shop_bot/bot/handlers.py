...
def get_user_router() -> Router:
    ...
    async def profile_handler_callback(callback: types.CallbackQuery):
        ...
        if not active_keys:
            ...
            # Usage is shown from subscription #1 only.
            usage_source_key = user_keys[0] if user_keys else active_keys[0]
            total_used_bytes = 0
            whitelist_used_bytes = 0
            whitelist_limit_gb = 0.0
            try:
                usage = await xui_api.get_key_usage_stats_from_host(usage_source_key) or {}
                used = max(
                    int(usage.get("upload_bytes", 0) or 0) + int(usage.get("download_bytes", 0) or 0),
                    0
                )
            except Exception:
                used = 0
            ...
            try:
                host = get_host(usage_source_key.get("host_name"))
                limit_gb = float(host.get("client_monthly_traffic_gb") or 0) if host else 0.0
            except Exception:
                limit_gb = 0.0
            if limit_gb > 0:
                whitelist_used_bytes = used
                whitelist_limit_gb = limit_gb

            total_used_gb = total_used_bytes / (1024 ** 3)
            whitelist_used_gb = whitelist_used_bytes / (1024 ** 3)
            final_text = (
                f"👤 <b>Профиль:</b> {username}\n\n"
                f"{vpn_status_line} <b>Статус VPN:</b>\n"
                f"⏳ <b>Осталось:</b> {remaining_text}\n\n"
                "🌐 <b>Использование трафика:</b>\n"
                f"├ Общий: {total_used_gb:.2f} ГБ / ∞\n"
                f"└ Белые списки: {whitelist_used_gb:.2f} ГБ / "
                f"{(f'{whitelist_limit_gb:.0f}' if whitelist_limit_gb > 0 else '∞')} ГБ\n\n"
