...
def get_user_router() -> Router:
    ...
    async def profile_handler_callback(callback: types.CallbackQuery):
        ...
        if not active_keys:
            ...
            for key in active_keys:
                try:
                    usage = await xui_api.get_key_usage_stats_from_host(key) or {}
                    used = max(
                ...
                try:
                    host = get_host(key.get("host_name"))
    ...
    async def referral_program_handler(callback: types.CallbackQuery):
        ...
        text = (
            ...
            "🗣 За каждого приведенного пользователя Вы получите 14 дней подписки, "
