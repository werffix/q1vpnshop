...
def create_webhook_app(bot_controller_instance):
    ...
    def activate_subscription_route(token: str):
        ...
        sub_url = f"{request.scheme}://{request.host}/sub/{clean_token}"
        ...
        # Primary path: official happ://crypt...
        # Fallback path: classic happ deeplinks.
        if happ_crypto_link:
            open_link = happ_crypto_link
            launch_variants = [happ_crypto_link]
        else:
            open_link = deeplink_raw
            launch_variants = [deeplink_raw, deeplink_encoded, deeplink_import, deeplink_sub]
...
<body>
  ...
  <script>
    const variants = [
      {json.dumps(launch_variants[0] if len(launch_variants) > 0 else "")},
      {json.dumps(launch_variants[1] if len(launch_variants) > 1 else "")},
      {json.dumps(launch_variants[2] if len(launch_variants) > 2 else "")},
      {json.dumps(launch_variants[3] if len(launch_variants) > 3 else "")}
    ];
    let i = 0;
    function tryOpen() {{
