...
def create_webhook_app(bot_controller_instance):
    ...
    def _serve_unified_subscription(token: str):
        ...
        if not merged_lines:
            ...
            # Expired users receive a dedicated placeholder node in subscription clients.
            # Active users never enter this branch.
    ...
    def add_host_route():
        ...
        for user in (get_all_users() or []):
            ...
            for key in keys:
                try:
                    ...
                    if is_expired_host or expiry_dt > now:
            ...
            try:
                ...
                if is_expired_host:
                    try:
                        asyncio.run(xui_api.set_client_enabled_on_host(host_name, host_email, enabled=False))
                    except Exception:
                        pass
                else:
        ...
        message = (
            ...
            f"{'Подготовлено отключенных клиентов для пользователей с подпиской' if is_expired_host else 'Пользователям с активной подпиской добавлено ключей'}: {migrated_users}."
        )
        if failed_users:
