...
def create_webhook_app(bot_controller_instance):
    ...
    def _resolve_user_id_from_any_token(raw_token: str):
    ...
    def _format_bytes_for_ui(value_bytes: int | float | None) -> str:
        try:
    ...
    @flask_app.route('/api/connect/<token>', methods=['GET'])
    def connect_data_route(token: str):
        token, user_id = _resolve_user_id_from_any_token(token)
        if user_id is None:
            return jsonify({"ok": False, "error": "invalid_token"}), 403

        user = get_user(int(user_id)) or {}
        username = user.get("username") or f"user_{user_id}"
        keys = get_keys_for_user(int(user_id)) or []
        now = datetime.now()

        latest_expiry_dt = None
        has_active = False
        for k in keys:
            raw = k.get("expiry_date")
            if not raw:
                continue
            try:
                ...
                continue
            if latest_expiry_dt is None or dt > latest_expiry_dt:
                latest_expiry_dt = dt
            if dt > now:
                has_active = True
        ...
        items = []
        for k in keys:
            host_name = (k.get("host_name") or "").strip()
            if not host_name:
                continue

            try:
                usage = asyncio.run(xui_api.get_key_usage_stats_from_host(k)) or {}
            except Exception:
                usage = {}
            upload_bytes = max(int(usage.get("upload_bytes", 0) or 0), 0)
            download_bytes = max(int(usage.get("download_bytes", 0) or 0), 0)
            total_bytes = max(int(usage.get("total_bytes", 0) or 0), 0)
            used = upload_bytes + download_bytes
            ...
            vless_url = None
            try:
                vless_url = asyncio.run(xui_api.build_vless_uri_for_key(k))
            except Exception:
                vless_url = None
            ...
            if vless_url:
                items.append({
                    "host_name": host_name,
                    "key_email": (k.get("key_email") or ""),
                    "vless_url": vless_url,
                    "used_text": _format_bytes_for_ui(used),
                    "total_text": _format_bytes_for_ui(total_bytes) if total_bytes > 0 else "∞",
                })

        req_host = (request.host.split(":", 1)[0] or "").strip()
        if req_host:
            subscription_url = f"{request.scheme}://{req_host}:8443/sub/{token}"
        else:
            subscription_url = f"{request.scheme}://{request.host}/sub/{token}"
        expire_text = latest_expiry_dt.strftime("%d.%m.%Y %H:%M") if latest_expiry_dt else "—"
        status_text = "Активна" if has_active else "Истекла"
        if total_quota_bytes > 0:
            traffic_text = f"{_format_bytes_for_ui(used_bytes_total)} / {_format_bytes_for_ui(total_quota_bytes)}"
        else:
            traffic_text = f"{_format_bytes_for_ui(used_bytes_total)} / ∞"
        ...
        return jsonify({
            ...
            "keys": items,
            "telegram_support_url": "https://t.me/q1support_bot",
        })

    @flask_app.route('/connect/<token>', methods=['GET'])
    def connect_page_route(token: str):
        token_clean, user_id = _resolve_user_id_from_any_token(token)
        if user_id is None:
            return Response("Invalid token", status=403, mimetype="text/plain")
        return render_template("connect_cabinet.html", token=token_clean)
