...
def create_webhook_app(bot_controller_instance):
    ...
    def activate_subscription_route(token: str):
        ...
        escaped_happ_link = html_escape.escape(happ_link, quote=True)
...
<body>
  <div class="wrap">
    <div class="card">
      ...
      <a class="btn btn-primary" href="{escaped_happ_link}">Открыть в Happ</a>
    </div>
  </div>
  <script>
    const happLink = {json.dumps(happ_link)};
    setTimeout(() => {{
      try {{ window.location.href = happLink; }} catch (_e) {{}}
    }}, 120);
  </script>
</body>
</html>"""
