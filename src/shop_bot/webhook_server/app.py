...
def create_webhook_app(bot_controller_instance):
    ...
    def activate_subscription_route(token: str):
        def _extract_happ_crypto_url(raw_text: str) -> str | None:
            ...
            # Plain text response: happ://crypt...
            if text.startswith("happ://crypt"):
                return text

            def _find_in_obj(obj):
                if isinstance(obj, str):
                    candidate = obj.strip()
                    return candidate if candidate.startswith("happ://crypt") else None
                if isinstance(obj, dict):
                    for value in obj.values():
                        found = _find_in_obj(value)
                        if found:
                            return found
            ...
            try:
                ...
                found = _find_in_obj(parsed)
                if found:
                    return found
            except Exception:
                pass

            # Last-resort extraction from mixed text.
        ...
        def _request_happ_crypto_url(sub_url: str, timeout_sec: float = 10.0) -> tuple[str | None, str | None]:
            ...
            attempts: list[tuple[bytes, dict[str, str], str]] = [
                (
                    json.dumps({"url": sub_url}, ensure_ascii=False).encode("utf-8"),
                    {
                ...
                (
                    urllib.parse.urlencode({"url": sub_url}).encode("utf-8"),
                    {
            ...
            for payload, headers, mode in attempts:
                ...
                try:
                    ...
                    try:
                        ...
                        pass
                    last_error = f"http_status_{status}({mode})"
                    if body:
                        crypto_url = _extract_happ_crypto_url(body)
                        if crypto_url:
                            return crypto_url, None
                    continue
                except Exception as e:
                    last_error = f"request_failed({mode}): {e}"
                    continue
                ...
                if crypto_url:
                    return crypto_url, None
                last_error = f"invalid_or_empty_response({mode})"
        ...
        escaped_sub_url = html_escape.escape(sub_url, quote=True)
