...
<body>
  <div class="wrap">
    <div class="card">
      ...
      <a class="btn btn-primary" href="{deeplink_raw}">Открыть в Happ</a>
  ...
  <script>
    const variants = [
      {json.dumps(deeplink_raw)},
      {json.dumps(deeplink_encoded)},
      {json.dumps(deeplink_import)},
      {json.dumps(deeplink_sub)}
    ];
    let i = 0;
    function tryOpen() {{
