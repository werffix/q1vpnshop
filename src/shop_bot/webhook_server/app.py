    ...
    def _serve_unified_subscription(token: str):
        ...
        for key in keys:
            try:
                ...
                # Preferred path: build direct VLESS URI from each host inbound/client.
                direct_vless = asyncio.run(xui_api.build_vless_uri_for_key(key))
                if direct_vless:
                    if direct_vless not in seen:
                        seen.add(direct_vless)
                        merged_lines.append(direct_vless)
                    continue

                # Fallback: read host subscription endpoint (legacy behavior).
