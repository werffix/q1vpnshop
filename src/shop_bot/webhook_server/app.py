...
def create_webhook_app(bot_controller_instance):
    ...
    def activate_subscription_route(token: str):
        ...
        open_link = happ_crypto_link or deeplink_raw
        launch_variants = [v for v in [happ_crypto_link, deeplink_raw, deeplink_encoded, deeplink_import, deeplink_sub] if v]
...
<body>
  ...
  <script>
    const variants = [
      ...
      {json.dumps(launch_variants[4] if len(launch_variants) > 4 else "")}
    ];
    let i = 0;
    function tryOpen() {{
