...
from shop_bot.data_manager.database import (
    ...
    move_host_order, update_host_is_expired)
...
def create_webhook_app(bot_controller_instance):
    ...
    def _serve_unified_subscription(token: str):
        ...
        if not merged_lines:
            ...
            expired_hosts = [h for h in (get_all_hosts() or []) if int(h.get("is_expired_host") or 0) == 1]
            ...
            for key in keys:
                try:
                    raw = key.get("expiry_date")
                    if raw:
                        expiry_anchor_ms = max(expiry_anchor_ms, int(datetime.fromisoformat(str(raw)).timestamp() * 1000))
                except Exception:
                    continue
            if expiry_anchor_ms <= 0:
                expiry_anchor_ms = int(datetime.now().timestamp() * 1000)
                ...
                try:
                    created = asyncio.run(
                        xui_api.create_or_update_key_on_host(
                            host_name=host_name,
                            email=email,
                            expiry_timestamp_ms=expiry_anchor_ms,
                            preferred_uuid=user_uuid
                        )
                    )
                    if not created:
                        continue
                    try:
                        asyncio.run(xui_api.set_client_enabled_on_host(host_name, email, enabled=True))
                    except Exception:
                        pass
                    direct = asyncio.run(
                        xui_api.build_vless_uri_for_key(
                    ...
                    if direct and direct not in expired_seen:
                        expired_seen.add(direct)
                        expired_lines.append(direct)
                except Exception as e:
                    logger.warning(f"Не удалось подготовить expired-host ссылку для user={user_id}, host='{host_name}': {e}")

            if expired_lines:
                payload = "\n".join(expired_lines) + "\n"
                encoded = base64.b64encode(payload.encode("utf-8")).decode("utf-8")
                response = Response(encoded, status=200, mimetype="text/plain")
                response.headers["profile-title"] = _header_utf8_via_latin1("Ваша подписка истекла")
            ...
            # Fallback placeholder only if there are no expired hosts configured.
            expired_name = urllib.parse.quote("🚫 Ваша подписка истекла!")
            expired_uri = (
