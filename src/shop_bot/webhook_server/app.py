...
def create_webhook_app(bot_controller_instance):
    ...
    def _ensure_clients_on_all_hosts(user_id: int, keys: list[dict]) -> list[dict]:
        ...
        try:
            ...
            if expiry_candidates_ms:
                anchor_expiry_ms = max(expiry_candidates_ms)
            else:
                # If DB has no key yet, create with default period anchor.
                anchor_expiry_ms = now_ms + (30 * 24 * 60 * 60 * 1000)
                logger.warning(f"Для user={user_id} не найден срок действия ключей. Использую fallback 30 дней.")

            if anchor_expiry_ms < now_ms:
                anchor_expiry_ms = now_ms + (30 * 24 * 60 * 60 * 1000)
                logger.warning(f"Для user={user_id} срок действия истёк. Использую fallback 30 дней.")
            ...
            for host in hosts:
                ...
                digest = hashlib.sha1(f"{user_id}:{host_name}".encode("utf-8")).hexdigest()[:12]
                email = f"u{user_id}.{digest}@bot.local"
                if get_key_by_email(email):
                    digest2 = hashlib.sha1(f"{user_id}:{host_name}:{time.time()}".encode("utf-8")).hexdigest()[:12]
                    email = f"u{user_id}.{digest2}@bot.local"
    ...
    def _serve_unified_subscription(token: str):
        ...
        for key in keys:
            try:
                ...
                try:
                    upload_total += max(int(usage_data.get("upload_bytes", 0) or 0), 0)
                    download_total += max(int(usage_data.get("download_bytes", 0) or 0), 0)
                    quota_total += max(int(usage_data.get("total_bytes", 0) or 0), 0)
                ...
                # Preferred path: read host subscription endpoint.
                # It contains native per-panel links that clients usually parse best.
                added_from_host_subscription = False
                details = asyncio.run(xui_api.get_key_details_from_host(key))
                sub_url = (details or {}).get("connection_string")
                native_sub_url = (details or {}).get("native_connection_string")
                if sub_url and (sub_url.startswith("http://") or sub_url.startswith("https://")):
                    sub_parsed = urllib.parse.urlparse(sub_url)
                    is_recursive_subscription = False
                    if sub_parsed.scheme == req_scheme and sub_parsed.netloc == req_netloc:
                        p = (sub_parsed.path or "").strip()
                        # recursive aliases: /sub/<token> and /<token>
                        if p.startswith("/sub/") or (p.count("/") == 1 and len(p.strip("/")) > 0):
                            is_recursive_subscription = True
                            logger.warning(f"Пропускаю рекурсивный URL подписки для key_id={key.get('key_id')}: {sub_url}")

                    fetch_url = sub_url
                    if is_recursive_subscription and native_sub_url and native_sub_url.startswith(("http://", "https://")):
                        fetch_url = native_sub_url
                        logger.info(f"Использую нативный URL панели для key_id={key.get('key_id')}: {fetch_url}")

                    if fetch_url and fetch_url.startswith(("http://", "https://")):
                        with urllib.request.urlopen(fetch_url, timeout=12) as response:
                            payload = response.read().decode("utf-8", errors="ignore")

                        for entry in _extract_subscription_entries(payload):
                            if entry in seen:
                                continue
                            seen.add(entry)
                            merged_lines.append(entry)
                            added_from_host_subscription = True

                if added_from_host_subscription:
                    continue

                # Fallback: build direct VLESS URI from inbound/client.
        ...
        # Hard fallback: if nothing collected, force-create clients on every host
        # and build direct VLESS links from inbound/client state.
        if not merged_lines:
            try:
                hosts = get_all_hosts() or []
                now_ms = int(datetime.now().timestamp() * 1000)
                anchor_expiry_ms = max(expiry_candidates) * 1000 if expiry_candidates else (now_ms + 30 * 24 * 60 * 60 * 1000)
        ...
        if not merged_lines:
            return Response("No subscription entries available", status=404, mimetype="text/plain")
        ...
        support_url = "https://t.me/q1vpn_support"
        expire_at = max(expiry_candidates) if expiry_candidates else 0
        response.headers["profile-title"] = urllib.parse.quote(profile_title, safe="")
        response.headers["support-url"] = support_url
        response.headers["subscription-userinfo"] = (
            f"upload={upload_total}; download={download_total}; total={quota_total}; expire={expire_at}"
        )
        return response
